...
from torch_geometric_signed_directed import Telegram
from definitions import ROOT_DIR
from .dataset import Dataset
from utils.third_party import load_snap_patents_mat, get_arxiv_year_dataset, get_ogbn_arxiv_dataset
...
def get_dataset(name, self_loop=False):
    data_root = osp.join(ROOT_DIR, 'datasets')
    if name in ['cornell', 'texas', 'wisconsin']:
        dataset = WebKB(root=data_root, name=name, transform=T.NormalizeFeatures())
    elif name in ['roman_empire_directed','chameleon_filtered', 'squirrel_filtered','questions_directed','questions','amazon_ratings_directed']:
        dataset = Dataset(name, add_self_loops=self_loop).data
        ...
        dataset = load_snap_patents_mat(n_classes=5)
    elif name in ["ogbn-arxiv"]:
        dataset = get_ogbn_arxiv_dataset()
    elif name == "arxiv-year":
        dataset = get_arxiv_year_dataset()
    elif name == 'telegram':
        dataset = Telegram('datasets/')
    else:
