...
class Planetoid(InMemoryDataset):
    ...
    def process(self):
        data = full_load_citation_2(self.name, self.raw_dir)
...
def create_sparse_adj_matrix(edge_list):
    edge_array = np.array(edge_list)
    row = edge_array[:, 0]
    col = edge_array[:, 1]
    data = np.ones(len(edge_list))

    # Compute the size of the matrix (assuming node indices are 0-based or close)
...
def full_load_citation_2(dataset_str, raw_dir):
    """Code adapted from https://github.com/Yujun-Yan/Heterophily_and_oversmoothing/blob/main/process.py#L33"""
    names = ['edges', 'features', 'labels']
    objects = []
    for i in range(len(names)):
        path = osp.join(raw_dir, "{}.{}.pickle".format(dataset_str, names[i]))
        with open(path, 'rb') as f:
           objects.append(pkl.load(f, encoding='latin1'))
    edges, features, labels = tuple(objects)
    
    sparse_mx = create_sparse_adj_matrix(edges).astype(np.float32)
    # Prepare in PyTorch Geometric Format
    #sparse_mx = create_sparse_adj_matrix(adj).astype(np.float32)
    indices = torch.from_numpy(np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64))
    shape = torch.Size(sparse_mx.shape)
    edge_index, _ = coalesce(indices, None, shape[0], shape[1])
