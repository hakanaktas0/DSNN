...
class Dataset:
    def __init__(self, name, add_self_loops=True, device='cpu', use_sgc_features=False, use_identity_features=False,
        ...
        data = np.load(os.path.join('datasets/', f'{name.replace("-", "_")}.npz'))
        ...
        self.data = Data(x=node_features, edge_index=edges, y=labels, train_mask=train_masks.T, val_mask=val_masks.T,
                    test_mask=test_masks.T, num_classes=num_classes)
